    ...
    public void play(){
        ...
        try{
         ...
         BufferedInputStream bin= new BufferedInputStream(bell);    
            ...
            clip.open(ais);           
            clip.start();
            a = (int)clip.getMicrosecondLength()/1000;
...
public void Bhawana(){
    try{
         ClassLoader CL = this.getClass().getClassLoader();
         InputStream bav=CL.getResourceAsStream("Bhawana.wav");
         BufferedInputStream bin1= new BufferedInputStream(bav);    
            AudioInputStream ais = AudioSystem.getAudioInputStream(bin1);
            
            Clip clip = AudioSystem.getClip();
            clip.open(ais);           
            clip.start();
            b = (int)clip.getMicrosecondLength()/1000;
            System.out.println(a);
            
            //for (int i=0;i< a;i++)
              //System.out.println("Wasting time because the audio runs in its own thread.");
        }
            catch(UnsupportedAudioFileException Exception ){} catch (IOException ex) {
                Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
            } catch (LineUnavailableException ex) {
...
public void damso(){
    try{
         ClassLoader CL = this.getClass().getClassLoader();
         InputStream damso=CL.getResourceAsStream("damso.wav");
         BufferedInputStream bin2= new BufferedInputStream(damso);    
            AudioInputStream ais = AudioSystem.getAudioInputStream(bin2);
            
            Clip clip = AudioSystem.getClip();
            clip.open(ais);           
            clip.start();
            c = (int)clip.getMicrosecondLength()/1000;
            System.out.println(a);
...
public void Final(){
     try{ 
        ClassLoader CL = this.getClass().getClassLoader();
         InputStream bell=CL.getResourceAsStream("final.wav");
         BufferedInputStream bin= new BufferedInputStream(bell);    
            AudioInputStream ais = AudioSystem.getAudioInputStream(bin);
            ...
            clip.open(ais);           
            clip.start();
            f = (int)clip.getMicrosecondLength()/1000;
            System.out.println(a);
